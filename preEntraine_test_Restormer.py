...
def process_single_image(input_filepath, out_filepath):
    ...
    with torch.no_grad():
      ...
      output_filepath = os.path.join(out_filepath, filename)

      cv2.imwrite(output_filepath, cv2.cvtColor(restored, cv2.COLOR_RGB2BGR))

      if not isCudaAvailable:
        del input_, restored

      return output_filepath  # Renvoie le nom du fichier traité
...
if __name__ == "__main__":
  ...
  print(processed_file)  # Imprime le nom du fichier traité
